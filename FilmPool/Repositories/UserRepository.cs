...
namespace FilmPool.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<User> AuthenticateUser(string userName, string password)
        {
            ...
            if (user != null)
            {
                user = user.Password == GetHashedPassword(password) ? user : null;
            }
            return user;
        }
        ...
        {
            string newhashedpassword = GetHashedPassword(user.Password);
        ...
        public async Task<bool> ResetPasswordAsync(User user, string token, string newPassword)
        {
            ...
            string newhashedpassword=GetHashedPassword(newPassword);
            if (IsValid)
            {
                User currentUser = await Get(user.Id);
                currentUser.FirstName = user.FirstName;
